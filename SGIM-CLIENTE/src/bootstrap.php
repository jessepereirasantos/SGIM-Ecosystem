...
try {
    ...
    // Adicionar colunas faltantes se a tabela já existia (Compatibilidade Resiliente)
    $colunas_novidades = $pdo->query("SHOW COLUMNS FROM sistema_novidades")->fetchAll(PDO::FETCH_COLUMN);
    if (!in_array('badge', $colunas_novidades)) {
        $pdo->exec("ALTER TABLE sistema_novidades ADD COLUMN badge VARCHAR(50) DEFAULT 'OTA' AFTER icone");
    }
    if (!in_array('data_criacao', $colunas_novidades)) {
        $pdo->exec("ALTER TABLE sistema_novidades ADD COLUMN data_criacao DATETIME DEFAULT CURRENT_TIMESTAMP AFTER data_lancamento");
    }
    ...
    // Soft Delete (Fase 3) - Adicionar colunas de forma compatível
    ...
    foreach ($tabelas_patch as $tabela) {
        try {
            ...
            if (!in_array('deleted_at', $cols)) {
                $pdo->exec("ALTER TABLE $tabela ADD COLUMN deleted_at TIMESTAMP NULL");
            }
        } catch (Exception $e) {}
    }
    ...
    // Patch para coluna 'grupo' se a tabela já existia (Compatibilidade)
    $cols_cfg = $pdo->query("SHOW COLUMNS FROM configuracoes")->fetchAll(PDO::FETCH_COLUMN);
    if (!in_array('grupo', $cols_cfg)) {
        $pdo->exec("ALTER TABLE configuracoes ADD COLUMN grupo VARCHAR(50) DEFAULT 'geral'");
    }

    // Inserir configurações padrão se não existirem
    $default_configs = [
        'versao_sistema' => '1.1.0',
        'master_url'     => 'https://escolateologicaeloha.com.br/',
        'license_key'    => ''
    ];
