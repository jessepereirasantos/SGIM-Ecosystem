...
try {
    ...
    if ($checkUpdate) {
        ...
        if (isset($ota['has_update']) && $ota['has_update']) {
            ...
            // TRAVA DE SEGURANÇA: Impedir reprocessamento se a versão for <= atual
            if (version_compare($ver, $currentVersion, '<=')) {
                error_log("[SGIM-OTA] [CLIENT] Bloqueio de reprocessamento: $ver <= $currentVersion");
        ...
        if (isset($ota['has_update']) && $ota['has_update']) {
            ...
            $ver = $ota['latest_version'] ?? $ota['latest'];
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-6">
            <div>
                <ul class="space-y-1">
                    ...
                    <li>
                        <a class="flex items-center gap-3 px-3 py-2.5 rounded-twelve bg-brand/10 text-brand font-bold border border-brand/20 transition-all" href="#">
                            <span class="material-symbols-outlined text-[20px]">support_agent</span>
                            <span class="text-sm">SUPORTE VIP TESTE</span>
                        </a>
                    </li>
                </ul>
            </div>
