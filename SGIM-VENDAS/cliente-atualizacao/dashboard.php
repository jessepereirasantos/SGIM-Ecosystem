...
if ($pdo) {
    ...
    try {
        ...
        if ($is_sqlite) {
            $stmtEntradas = $pdo->query("SELECT SUM(valor) FROM financeiro_transacoes WHERE tipo = 'entrada' AND strftime('%m', data_cadastro) = strftime('%m', 'now')");
            $stmtSaidas = $pdo->query("SELECT SUM(valor) FROM financeiro_transacoes WHERE tipo = 'saida' AND strftime('%m', data_cadastro) = strftime('%m', 'now')");
        } else {
            $stmtEntradas = $pdo->query("SELECT SUM(valor) FROM financeiro_transacoes WHERE tipo = 'entrada' AND MONTH(data_cadastro) = MONTH(NOW())");
            $stmtSaidas = $pdo->query("SELECT SUM(valor) FROM financeiro_transacoes WHERE tipo = 'saida' AND MONTH(data_cadastro) = MONTH(NOW())");
        }
